...
finetuned_model_backbone_mapping_dict = {
    "Llama-2-7b-instruct": "Llama-2-7b-hf",
...
def recover_from_pretrained_model(finetuned_model_name, pretrained_model_name, args, logger: logging.Logger, recovered_model_save_path: str, recover_manner: str):
    try:
        pretrained_model = AutoModelForCausalLM.from_pretrained(pretrained_model_name_or_path=os.path.join(cache_dir(pretrained_model_name), pretrained_model_name), device_map="cpu")
        pretrained_tokenizer = AutoTokenizer.from_pretrained(pretrained_model_name_or_path=os.path.join(cache_dir(pretrained_model_name), pretrained_model_name))
        finetuned_model = AutoModelForCausalLM.from_pretrained(pretrained_model_name_or_path=os.path.join(cache_dir(finetuned_model_name), finetuned_model_name), device_map="cpu")
        finetuned_tokenizer = AutoTokenizer.from_pretrained(pretrained_model_name_or_path=os.path.join(cache_dir(finetuned_model_name), finetuned_model_name))
    except:
        pretrained_model = AutoModelForCausalLM.from_pretrained(pretrained_model_name_or_path=pretrained_model_name, cache_dir=cache_dir(pretrained_model_name), device_map="cpu")
        pretrained_tokenizer = AutoTokenizer.from_pretrained(pretrained_model_name_or_path=pretrained_model_name, cache_dir=cache_dir(pretrained_model_name))
        finetuned_model = AutoModelForCausalLM.from_pretrained(pretrained_model_name_or_path=finetuned_model_name, cache_dir=cache_dir(finetuned_model_name), device_map="cpu")
        finetuned_tokenizer = AutoTokenizer.from_pretrained(pretrained_model_name_or_path=finetuned_model_name, cache_dir=cache_dir(finetuned_model_name))
...
def create_llm(finetuned_model_name, pretrained_model_name, args, logger: logging.Logger, tensor_parallel_size=1, just_inference=False, save_model_path=None):
    if just_inference:
        if os.path.exists(os.path.join(cache_dir(finetuned_model_name), finetuned_model_name)):
            model_path = os.path.join(cache_dir(finetuned_model_name), finetuned_model_name)
            llm = LLM(model=os.path.join(cache_dir(finetuned_model_name), finetuned_model_name), tensor_parallel_size=tensor_parallel_size)
        else:
...
def test_alpaca_eval(llm, finetuned_model_name, args, logger: logging.Logger, start_index=0, end_index=sys.maxsize,
    ...
    # 1. Please replace the following path with the actual path to your alpaca_eval.json file
    your_local_json_path = "/home/user/project/math_code_data/alpaca_eval.json"

    # 2. Load using "json" mode and access the data with ["train"]
    #    This is because when loading directly from a local file, the default dataset name is "train"
    logger.info(f"Loading evaluation data from local file: {your_local_json_path}")
    eval_set = datasets.load_dataset("json", data_files=your_local_json_path)["train"]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--finetuned_model_name", type=str, default="WizardLM-13B-V1.2", help="name of the finetuned language model",
                        choices=["WizardLM-7B-V1.0", "WizardLM-13B-V1.2", "WizardLM-70B-V1.0",
                                 ...
                                 "llama-2-13b-code-alpaca","llama2_7b_code","Llama-2-7b-instruct","Llama-2-13b-hf","Llama-2-7b-hf"])
    ...
    if args.finetuned_model_name == "WizardLM-7B-V1.0":
        ...
        recovered_model_save_path = os.path.join(cache_dir(args.finetuned_model_name), f"{args.finetuned_model_name}-recovered")
