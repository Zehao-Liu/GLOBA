...
def get_param_names_to_merge(input_param_names: list, exclude_param_names_regex: list):
    ...
    for param_name in input_param_names:
        ...
        if exclude:
            print(f"Excluding parameter: {param_name}")  # 打印被排除的参数
        else:
            print(f"Including parameter: {param_name}")  # 打印被包含的参数
...
def smart_tokenizer_and_embedding_resize(
    ...
    if num_new_tokens > 0:
        # 动态调整嵌入层
        model.resize_token_embeddings(current_vocab_size + num_new_tokens)

        # 初始化新token的嵌入
        input_embeddings = model.get_input_embeddings().weight.data
        output_embeddings = model.get_output_embeddings().weight.data

        # 使用已有token的平均值初始化新token（排除padding token）
        valid_tokens = input_embeddings[:current_vocab_size]  # 修改索引范围
        input_embeds_avg = valid_tokens.mean(dim=0, keepdim=True)
        output_embeds_avg = output_embeddings[:current_vocab_size].mean(dim=0, keepdim=True)

        input_embeddings[-num_new_tokens:] = input_embeds_avg
        output_embeddings[-num_new_tokens:] = output_embeds_avg
